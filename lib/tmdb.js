...
// than theatrical release dates or recurring TV broadcasts. We also
// require the movie's original (primary) release date to be fairly
// recent, so long-ago catalog titles that pick up a new digital reissue
// date (e.g. an old franchise re-released ahead of a sequel) don't show
// up looking like a brand-new release.
async function getNewReleases() {
  ...
  const earliestOriginalRelease = new Date();
  earliestOriginalRelease.setFullYear(today.getFullYear() - 3);
  ...
  url.searchParams.set("primary_release_date.gte", fmt(earliestOriginalRelease));
